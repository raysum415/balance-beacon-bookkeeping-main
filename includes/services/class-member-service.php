...
/**
 * Handles user-owned member records.
 */
final class Balance_Beacon_Member_Service {
	public function get_all( $book_id, $user_id ) {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT id, book_id, name, is_hidden, created_at, updated_at FROM {$tables['members']} WHERE book_id = %d AND user_id = %d ORDER BY name ASC, id ASC",
				$book_id,
				absint( $user_id )
			),
			ARRAY_A
	...
	public function create( array $data, $user_id ) {
		...
		if ( false === $inserted ) {
			throw new RuntimeException( 'Unable to create member.' );
		}
	...
	public function update( $id, array $data, $user_id ) {
		...
		if ( false === $updated ) {
			throw new RuntimeException( 'Unable to update member.' );
		}
	...
	public function delete( $id, $user_id ) {
		...
		$in_use = $wpdb->get_var( $wpdb->prepare( "SELECT 1 FROM {$tables['transactions']} WHERE member_id = %d AND user_id = %d LIMIT 1", $id, $user_id ) );
		if ( ! $in_use ) {
			$in_use = $wpdb->get_var( $wpdb->prepare( "SELECT 1 FROM {$tables['transaction_members']} tm INNER JOIN {$tables['transactions']} t ON t.id = tm.transaction_id WHERE tm.member_id = %d AND t.user_id = %d LIMIT 1", $id, $user_id ) );
		}
		if ( $in_use ) {
		...
		if ( false === $deleted ) {
			throw new RuntimeException( 'Unable to delete member.' );
		}
	...
	private function record_belongs_to_user( $id, $user_id ) {
		...
		return (bool) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT m.id FROM {$tables['members']} m INNER JOIN {$tables['books']} b ON b.id = m.book_id WHERE m.id = %d AND m.user_id = %d AND b.user_id = %d",
				$id,
				$user_id,
				$user_id
			)
